...
/**
 * Animated Bar Chart with Bausen styling
 */
  ...
  const option = useMemo(() => {
    ...
    const baseOptions: EChartsOption = {
      ...
      grid: {
        left: '3%',
        right: '4%',
        bottom: enableZoom ? '15%' : (isMultiSeries ? '10%' : '3%'),
        top: title ? '12%' : '8%',
        containLabel: true,
      },
      xAxis: horizontal ? {
    ...
    if (stack) {
      (baseOptions as Record<string, unknown>).series = (seriesData as Array<{ stack?: string }>).map(s => ({ ...s, stack: 'Total' }));
    } else {
      (baseOptions as Record<string, unknown>).series = seriesData;
    }

    return baseOptions;
  }, [categories, data, title, theme, horizontal, enableZoom, palette, valueFormatter, barRadius, barWidth, stack, showLabels]);
