...
interface DeletePressModalProps {
  title: string;
  theme: 'light' | 'dark';
  loading?: boolean;
  onCancel: () => void;
...
export default function DeletePressModal({ title, theme, loading, onCancel, onConfirm }: DeletePressModalProps) {
  return (
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              <TranslateText text="¿Eliminar comunicado?" />
            </h3>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="mb-6">
          <p className={`text-sm mb-3 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            <TranslateText text="Estás a punto de eliminar el siguiente comunicado:" />
          </p>
